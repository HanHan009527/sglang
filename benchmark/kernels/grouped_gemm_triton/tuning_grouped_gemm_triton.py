...
def benchmark_config(
    ...
    logger.info(f"Parameters: num_experts_per_partition={num_experts_per_partition}, hidden_size={hidden_size}, dtype={dtype}")
    logger.info(f"Quantization: use_fp8_w8a8={use_fp8_w8a8}, use_int8_w8a8={use_int8_w8a8}, use_int8_w8a16={use_int8_w8a16}")
    if block_shape:
    ...
    with override_config(config):
        grouped_gemm_triton(
            gateup_input, w13_weight, gateup_output, num_experts_per_partition, True, seg_indptr, weight_indices, 
            use_fp8_w8a8, scale_a, scale_b, block_shape
        )
        ...
        for config in tqdm(search_space):
            try:
                ...
                if kernel_time < best_time:
                    ...
                    best_config = config
                    logger.info(f"New best config: {best_config} with kernel_time={best_time:.2f} us")
            except Exception as e:
                logger.warning(f"Failed to benchmark config {config}: {e}")
                continue
        
        now = datetime.now()
        logger.info(f"{now.ctime()} Completed tuning for num_experts_per_partition={num_experts_per_partition}, hidden_size={hidden_size}")
        # assert best_config is not None
        return best_config, best_time


def distribute_search_space(search_space: List[Dict[str, int]], num_workers: int) -> List[List[Dict[str, int]]]:
    ...
    while len(search_chunks) < num_workers:
        ...
        largest_chunk_idx = max(range(len(search_chunks)), key=lambda i: len(search_chunks[i]))
...
def load_model_config(model_path: str, args: argparse.Namespace) -> Tuple[AutoConfig, int, torch.dtype, int, int, Optional[List[int]], bool, bool, bool]:
    ...
    try:
        ...
        logger.info(f"Using num_experts_per_partition (n_routed_experts/ep_size) from model config: {num_experts_per_partition} = {n_routed_experts}/{ep_size}")
        ...
        return model_config, hidden_size, dtype, top_k, num_experts_per_partition, block_shape, use_fp8_w8a8, use_int8_w8a8, use_int8_w8a16
...
def main():
    ...
    parser.add_argument("--model", type=str, default="/data00/models/DeepSeek-R1/", help="Path to model config or directory")
    parser.add_argument("--log-file", type=str, default="tuning_grouped_gemm.log", help="Log file path")
    parser.add_argument("--log-level", type=str, default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], help="Log level")
    parser.add_argument("--ep-size", type=int, default=16, help="Expert parallelism size")
    ...
    logger.info(f"CUDA is available. Found {torch.cuda.device_count()} GPUs: {[torch.cuda.get_device_name(i) for i in range(torch.cuda.device_count())]}")
    
    # 加载模型配置
    try:
        model_config, hidden_size, dtype, top_k, num_experts_per_partition, block_shape, use_fp8_w8a8, use_int8_w8a8, use_int8_w8a16 = load_model_config(args.model, args)
    except Exception as e:
        logger.error(f"Failed to load model config: {e}")
        return
    
    logger.info(f"Tuning grouped GEMM Triton kernel with hidden_size={hidden_size}, dtype={dtype}, num_experts_per_partition={num_experts_per_partition}")
    if block_shape:
    ...
    try:
        ...
        for tokens_per_expert in TOKENS_PER_EXPERT_VALUES:
            ...
            logger.info(f"Start tuning over {len(search_space)} configurations for tokens_per_expert={tokens_per_expert}...")
            ...
            logger.info(f"New best config for tokens_per_expert={tokens_per_expert}: {best_config} with kernel_time={best_time:.2f} us")
            logger.info(f"Tuning for tokens_per_expert={tokens_per_expert} took {end - start:.2f} seconds")
            ...
            try:
                ...
                logger.info(f"Verified latency for tokens_per_expert={tokens_per_expert}: {latency:.2f} us")
            except Exception as e:
                logger.error(f"Failed to benchmark best config for tokens_per_expert={tokens_per_expert}: {e}")
                import traceback
                logger.error(traceback.format_exc())
    finally:
        # 关闭Ray
