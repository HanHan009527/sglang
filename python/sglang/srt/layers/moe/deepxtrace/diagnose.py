...
from .deepxtrace_heatmap import plot_deepxtrace_heatmap
...
class Diagnose:
    ...
    def __init__(
        ...
        if self.enable_ll_diagnose:
            ...
            self.async_ll_step_counter = np.uint64(0)
        ...
        if self.rank == 0 and (not torch.cuda.is_current_stream_capturing()):
            ...
            for i, name in enumerate(["Dispatch", "Combine"]):
                ...
                plot_deepxtrace_heatmap(
                    matrix=stats_arr[:, i, :],
                    output_filename=f"deepxtrace.{name}.{self.async_ll_step_counter}",
                )
        return results
