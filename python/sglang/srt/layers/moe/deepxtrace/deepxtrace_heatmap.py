...
def plot_deepxtrace_heatmap(
    ...
    # Mask diagonal elements (set to NaN) to exclude them from visualization
    # This prevents zero diagonal values from interfering with color scale
    matrix_masked = matrix.copy().astype(float)
    np.fill_diagonal(matrix_masked, np.nan)

    # Create colormap and normalize data (excluding diagonal NaN values)
    cmap = create_optimized_ryg_cmap()
    log_matrix = np.log1p(matrix_masked)
    # Calculate normalization excluding NaN values
    valid_values = log_matrix[~np.isnan(log_matrix)]
    norm = plt.Normalize(vmin=valid_values.min(), vmax=valid_values.max())
