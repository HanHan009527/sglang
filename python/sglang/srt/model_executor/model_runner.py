...
class ModelRunner:
    """
    负责管理和执行模型的前向传播，是整个模型推理流程的核心控制器。

    `ModelRunner` 封装了从模型加载到实际执行的所有复杂细节，旨在提供一个高效、
    可配置的推理环境。它不仅处理单个模型的运行，还协调分布式环境下的多GPU协作、
    内存管理、以及各种性能优化策略。

    主要职责:
    1.  **环境初始化**:
        -   设置分布式环境，包括张量并行（TP）、流水线并行（PP）和数据并行（DP），
            并初始化 `torch.distributed`。
        -   根据设备类型（CPU/GPU）进行特定的环境配置，如线程绑定。

    2.  **模型加载与配置**:
        -   根据 `ModelConfig` 和服务参数 `ServerArgs` 加载指定的语言模型。
        -   应用模型特定的调整和配置，例如量化（torchao）、张量并行等。

    3.  **资源管理**:
        -   初始化和管理KV缓存池（`KVCachePool`），这是高效推理的关键，用于存储
            生成过程中的键值对（key-value pairs）。
        -   管理GPU内存，支持静态内存分配和CPU offload，以在有限的硬件资源下
            运行更大的模型或处理更多的请求。

    4.  **后端与优化**:
        -   初始化专门的计算后端，特别是注意力机制的后端（`AttentionBackend`），
            以加速计算密集型的自注意力操作。
        -   支持并初始化CUDA Graphs，将多个计算步骤编译成一个单一的图，从而显著
            减少GPU的启动开销，提升高并发场景下的吞吐量。

    5.  **高级功能支持**:
        -   **混合专家模型 (MoE)**: 初始化并管理与MoE相关的功能，包括专家位置的
            元数据、动态负载均衡（EPLB）和专家权重的物理迁移。
        -   **投机性解码 (Speculative Decoding)**: 配置和管理用于加速解码过程的
            小模型（draft model）。
        -   **LoRA**: 支持动态加载和管理LoRA适配器，以在不修改基础模型的情况下
            实现模型微调。

    6.  **执行入口**:
        -   提供 `forward_step` 和 `forward_prefill` 等核心方法，分别用于执行
            模型的解码步骤（生成单个token）和预填充步骤（处理初始prompt）。
        -   这些方法会根据当前的请求批次，准备输入数据，调用底层模型进行计算，
            并返回输出结果。
    """
