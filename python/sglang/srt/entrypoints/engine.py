...
def _compute_parallelism_ranks(
    ...
    # DWDP mode: tp_size=1, ep_size=dwdp_size. Each rank is its own EP rank.
    # The standard formula would divide by zero since ep_size > tp_size.
    if getattr(server_args, "dwdp_size", 1) > 1:
        return attn_cp_rank, 0, tp_rank % server_args.ep_size
