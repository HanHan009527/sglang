...
RUN CPPFLAGS="-I/usr/local/cuda/include" pip3 install https://developer.download.nvidia.com/compute/nvshmem/redist/nvshmem_python/source/nvshmem_python-source-0.1.0.36132199_cuda12-archive.tar.xz
...
RUN apt-get update && apt-get install -y --no-install-recommends wget gnupg \
    ...
    && apt-get update && apt-get install -y --no-install-recommends clang-19 llvm-19 libclang-19-dev \
